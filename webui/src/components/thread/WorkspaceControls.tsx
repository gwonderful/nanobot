    ...
    <DropdownMenu>
      <DropdownMenuTrigger asChild disabled={disabled || !onChange}>
        <Button
          ...
          className={cn(
            "workspace-access-button max-w-[min(12.5rem,42vw)] rounded-[10px] border border-transparent font-semibold shadow-none",
            isHero ? "h-8 px-2.5 text-[12px]" : "h-9 px-3 text-[12.5px]",
            isFull
              ? "workspace-access-button--full"
...
function AccessMenuItem({
  ...
  return (
    <DropdownMenuItem
      ...
      onSelect={onSelect}
      className={cn(
        "workspace-access-menu-item flex h-10 items-center gap-3 rounded-xl px-3 text-[13.5px] font-semibold",
        warning && "workspace-access-menu-item--warning",
      )}
    >
